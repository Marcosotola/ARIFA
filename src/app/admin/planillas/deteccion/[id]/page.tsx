...
interface FilaChecklist { itemId: string; descripcion: string; esGrupo?: boolean; valor: string; observacion: string; }
    ...
    if (checked) {
      const nueva: PlanillaEnOT = {
        ...
        filasChecklist: p.tipo === "checklist"
          ? (p.items || []).map(it => ({ itemId: it.id, descripcion: it.descripcion, esGrupo: it.esGrupo, valor: "", observacion: "" }))
          : [],
  ...
  const handlePDF = async () => {
    ...
    for (const p of planillasEnOT) {
      ...
      if (p.tipo === "checklist") {
        autoTable(pdf, {
          startY: y, margin: { left: ML, right: MR },
          head: [["Ítem", "OK", "NOK", "N/A", "Observaciones"]],
          body: p.filasChecklist.map(f => f.esGrupo
            ? [{ content: f.descripcion, colSpan: 5, styles: { fillColor: [220, 220, 220], fontStyle: "bold", textColor: [30, 30, 30] } }]
            : [f.descripcion, f.valor === "ok" ? "✓" : "", f.valor === "nok" ? "✓" : "", f.valor === "na" ? "✓" : "", f.observacion]),
          styles: { fontSize: 8 }, headStyles: { fillColor: [0, 34, 68] },
          columnStyles: { 1: { halign: "center", cellWidth: 12 }, 2: { halign: "center", cellWidth: 12 }, 3: { halign: "center", cellWidth: 12 } },
        });
      } else {
        autoTable(pdf, {
          startY: y, margin: { left: ML, right: MR },
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.88rem" }}>
                    <thead>
                      <tr style={{ background: "#002244", color: "#fff" }}>
                        ...
                        <th style={{ textAlign: "center", padding: "10px 8px", minWidth: 50 }}>OK</th>
                        <th style={{ textAlign: "center", padding: "10px 8px", minWidth: 50 }}>NOK</th>
